export const UserLoader = `const Dataloader = require('dataloader')
...
const userLoader = new Dataloader(async userIds => {
    const users = await db.query(\`SELECT (id, events, fullName, profilePic) 
     FROM users WHERE id IN \${userIds}\`)
    // Suppose our DB/ORM has a penchant for anarchy and returns the array 
    // of objects of {id, events, fullName, profilePic} in an order not matching our 
